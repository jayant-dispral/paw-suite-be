...
func (wp *WorkerPool) apiWorker(id int) {
	...
	for {
		...
		case task := <-wp.apiWorkQueue:
			...
			if result == nil {
				fmt.Printf("[API worker %d], WARNING result empty", id)
...
func (wp *WorkerPool) processingWorker(id int) {
	...
	for {
		...
		case result := <-wp.processingQueue:
			...
			//check if there are no social posts in result:
			if len(result.SocialPosts) < 1 {
				fmt.Printf("[Processing Worker %d] WARNING: no social posts in result with task id: %v\n", id, result.TaskID)
				continue
			}
